...
TRAIN_SESSIONS = {
    "adjudicator-100-2026-08-28",
    "adjudicator-100-2026-08-28b",
    "adjudicator-100-2026-08-28c",
...
def main() -> None:
    ...
    session_id = args.session or (
        f"adjudicator-eval-100-{datetime.now(timezone.utc).strftime('%Y%m%d')}"
    )
    if session_id in TRAIN_SESSIONS or session_id.startswith("adjudicator-100-"):
        print(
            f"Hard failure: session {session_id!r} collides with the training corpus",
            file=sys.stderr,
        )
        sys.exit(1)
